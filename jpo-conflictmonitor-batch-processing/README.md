...
MongoDB via Kafka Connect, see `jpo-utils`). That model, reacting to a continuous stream of
...
3. Maps the SPaT message's `signalGroupId` to the ATSPM controller's phase number. This
   ...
   overlapping phase scenarios. A signal group with no explicit override defaults to the
   ATSPM phase of the same number, and an override is only needed when that assumption
   doesn't hold.
4. Pairs each SPaT signal indication change with the nearest matching ATSPM controller
   event within a small time window, and computes, per signal group, the percentage of
...
- `mongo`: `ProcessedSpatCollectionUpdater`, which the scheduled task runs before each
  ...
  (`utcTimeStampTS`) so this project can run efficient time-range queries against it.
...
- `cm.batch.atspm.spat.validation.routes`: The list of routes/signals/intersections to
  ...
  behavior). A `phases` entry is only needed for a signal group whose ATSPM phase number
  differs from its SPaT signal group ID, or that has a secondary/overlap phase. An
  unlisted signal group defaults to the ATSPM phase of the same number. A signal can be
  `enabled: false` to configure it (e.g. document why it's disabled), e.g. if MAP or ATSPM
  data isn't yet available for that intersection. Note that this currently only stops the
  route's task from running at all if *every* signal on the route is disabled. An
  individual disabled signal on an otherwise-enabled route is not yet filtered out of that
  route's queries.
- `cm.batch.clock.*`: See `time` package description above. **Check this before deploying.**
  The checked-in `application.yaml` default is `offset: true` with a fixed
  `start-timestamp`, intended for local testing against historical data. Set `offset:
  false` (there is no environment variable override for this one) for normal operation, or
  the app will keep querying the same fixed historical window forever instead of advancing
  with real time.
